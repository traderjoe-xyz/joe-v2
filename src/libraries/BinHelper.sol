...
library BinHelper {
    ...
    {
        unchecked {
            uint256 pow = Constants.DOUBLE_PRICE_PRECISION / _getBPValue(_bp);
            uint256 absId = _id >= 0 ? uint256(_id) : uint256(-_id);

            if (absId > 0xca62c) revert BinHelper__IdOverflows(_id);

            uint256 result = Constants.PRICE_PRECISION;

            if (absId & 0x1 != 0)
                result = (result * pow) / Constants.PRICE_PRECISION;
            pow = (pow**2) / Constants.PRICE_PRECISION;
            if (absId & 0x2 != 0)
                result = (result * pow) / Constants.PRICE_PRECISION;
            pow = (pow**2) / Constants.PRICE_PRECISION;
            if (absId & 0x4 != 0)
            ...
            return
                _id <= 0 ? result : Constants.DOUBLE_PRICE_PRECISION / result;
        }
    }
    ...
    function _getBPValue(uint256 _bp) internal pure returns (uint256) {
        ...
        return Constants.PRICE_PRECISION + _bp * 1e32;
    }
}
